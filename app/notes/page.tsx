...
export default async function Page({
  ...
  const { api, session } = await getServerClient(`/notes`);
  const { data: notes } =
  ...
  return (
    <main className="h-full">
      <div className="mt-5 flex flex-row content-center justify-center">
        <Search
          className={clsx(
            (!search || !search.length) && (!notes || notes.length === 0)
              ? "invisible"
              : undefined,
          )}
        />
      ...
      {!notes || notes.length === 0 ? (
        ...
        <div>
          <ul
            role="list"
            className="mx-6 mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 "
          >
